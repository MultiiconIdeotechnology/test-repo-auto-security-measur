...
export class ProfileInsuranceComponent extends BaseListingComponent implements OnChanges {
  ...
  getSupplierCombo(): void {
    this.cacheService.getOrAdd(CacheLabel.getFareypeSupplierInsuranceCombo,
      this.supplierInventoryProfileService.getFareypeSupplierBoCombo('Insurance', '')).subscribe({
        next: data => {
  ...
  submit(): void {
    ...
    const newInventory = {
      service: formValue.service || 'Insurance',
