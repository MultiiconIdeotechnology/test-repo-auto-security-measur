...
export class AirlineCareerWiseComponent
    ...
    refreshItems(event?: any): void {
        ...
        const request = {};
        request['From_Date'] = DateTime.fromJSDate(
            this.startDate.value
        ).toFormat('yyyy-MM-dd');
        request['To_Date'] = DateTime.fromJSDate(this.endDate.value).toFormat(
            'yyyy-MM-dd'
        );
    ...
    isMaxValue(data: any, value: number): boolean {
        ...
        for (const key of Object.keys(data)) {
            if (typeof data[key] == 'number' && key != 'total' && data[key] != 0) {
                list.push(data[key]);
            }
        }
    ...
    isMinValue(data: any, value: number): boolean {
        const list = [];
        for (const key of Object.keys(data)) {
            if (typeof data[key] == 'number' && key != 'total' && data[key] != 0) {
                list.push(data[key]);
    ...
    public updateDate(event: any, isRefresh: boolean = true): void {
        ...
        if (isRefresh) this.refreshItems();
    }
